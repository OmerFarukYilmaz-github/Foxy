...
public class Slammer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (Vector3.Distance(slammerTarget.position, PlayerController.instance.transform.position) < 1.6f
             || Vector3.Distance(slammerTarget.position, RaycastDeneme.instance.roboComponent.transform.position) < 1.6f)
            {
                slamming = true;
